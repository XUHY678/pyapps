...
def dd_login_status(uname, has_dd=False):
    while True:
        if has_dd:
            dname, pwd = load_dingding("{}/user/{}/dingding".format(data_path, uname))
            print("读取用户信息成功")
        else:
            dname, pwd = get_dd()
        driver_login = driver.ChromeDriver(noimg=False)
        login_status = driver_login.dd_login(dname, pwd)
        if login_status:
            save_dingding("{}/user/{}/dingding".format(data_path, uname), dname, pwd)
